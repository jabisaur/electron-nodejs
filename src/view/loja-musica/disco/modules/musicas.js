...
export async function verMusicasDoDisco(discoId) {
    try {
        ...
        document.querySelector('#secao-musicas h2').innerHTML = `<i class="bi bi-music-note-beamed"></i> Músicas do Disco: ${disco.nome}`;
...
export async function carregarMusicasDoDisco(discoId) {
    try {
        ...
        if (!musicas || musicas.length === 0) {
            elements.listaMusicasDisco.innerHTML = '<p class="text-muted"><i class="bi bi-music-note"></i> Nenhuma música adicionada a este disco ainda.</p>';
            return;
        }
        ...
        musicas.forEach(musica => {
            html += `
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    <div>
                        <span class="badge bg-secondary me-2"><i class="bi bi-hash"></i> ${musica.ordem || '?'}</span>
                        <strong>${musica.nome}</strong> 
                        ${musica.duracao ? `<span class="badge bg-light text-dark ms-2"><i class="bi bi-clock"></i> ${musica.duracao}</span>` : ''}
                        ${musica.estilo_nome ? `<br><small class="text-muted"><i class="bi bi-tag"></i> ${musica.estilo_nome}</small>` : ''}
                    </div>
        ...
        console.error('Erro ao carregar músicas do disco:', erro);
        elements.listaMusicasDisco.innerHTML = '<p class="text-danger"><i class="bi bi-exclamation-triangle"></i> Erro ao carregar músicas.</p>';
    }
}
...
export async function removerMusicaDoDisco(discoId, musicaId) {
    ...
    try {
        ...
        window.dialog.exibirDialogMensagem({
            titulo: 'Sucesso',
            mensagem: '<i class="bi bi-check-circle"></i> Música removida do disco com sucesso!'
        });
    } catch (erro) {
        console.error('Erro ao remover música:', erro);
        window.dialog.exibirDialogMensagem({
            titulo: 'Erro',
            mensagem: '<i class="bi bi-exclamation-triangle"></i> Erro ao remover música: ' + erro.message
        });
    }
}